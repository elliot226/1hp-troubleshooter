...
export const freeExercisesByPainRegion = {
  wristFlexors: ["wristFlexorStretch", "dbWristFlexion", "isometricWristFlexion"],
  wristExtensors: ["wristExtensorStretch", "isometricWristExtension", "dbWristExtension"],
  thumbFlexors: ["thumbFlexorStretch", "isometricThumbFlexion", "thumbFlexionWithBand"],
  thumbExtensors: ["thumbExtensorStretch", "isometricThumbExtension", "dbRadialDeviation"],
  ulnarSideExtensors: ["wristExtensorStretch", "isometricUlnarDeviation", "dbUlnarDeviation"],
  ulnarSideFlexors: ["wristFlexorStretch", "isometricWristFlexion", "isometricWristUlnarDeviation"],
  pinkyFlexors: ["wristFlexorStretch", "dbWristFlexion", "isometricWristFlexion"],
  fingers: ["isometricWristFlexion", "lumbricalStretch", "fingerExtensionWithBand"]
};
...
/**
 * Check if an exercise is free for a user based on their pain regions
 * @param {String} exerciseId - Exercise ID to check
 * @param {Array} userPainRegions - Array of user's pain region IDs
 * @returns {Boolean} - True if the exercise should be free for this user
 */
export function isExerciseFreeForUser(exerciseId, userPainRegions) {
  if (!userPainRegions || userPainRegions.length === 0) {
    return false;
  }
  
  // Check if the exercise is free for any of the user's pain regions
  for (const regionId of userPainRegions) {
    const freeExercises = freeExercisesByPainRegion[regionId] || [];
    if (freeExercises.includes(exerciseId)) {
      return true;
    }
  }
  ...
  const allExercises = getExercisesForMultiplePainRegions(painRegions);
   ...
   Object.keys(program).forEach(category => {
    const categoryExercises = allExercises.filter(ex => {
      ...
      // If pro user, include all exercises
      if (proUser) return true;
      
      // For free users, check if it's a free exercise for their pain regions
      return isExerciseFreeForUser(ex.id, painRegions);
    });
