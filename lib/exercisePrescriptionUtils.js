 ...
 */
export async function updateExerciseTracking(userId, exerciseId, trackingData) {
  try {
    ...
    const newInstance = {
      ...
      timeOfDay,
      completed,
      repsPerformed: completed ? parseInt(repsPerformed, 10) || 0 : null,
      painLevel: completed ? parseInt(painLevel, 10) || 0 : null,
      weight: prescription.currentWeight
    };
    
    // Update prescription document
    const trackingInstances = [...(prescription.trackingInstances || []), newInstance];
    ...
    if (painLevel >= 7) {
      evaluateExerciseProgression(userId, exerciseId);
      return {...prescription, trackingInstances, consecCompletions, scalingEligible: true};
    }
}
