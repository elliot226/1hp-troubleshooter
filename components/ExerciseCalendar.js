  ...
  useEffect(() => {
    ...
    const fetchCompletionData = async () => {
      ...
      while (currentDate <= endDate) {
        ...
        try {
          const trackingData = await getExerciseTrackingForDate(currentUser.uid, currentDate);
    ...
    if ((dayData.amCompleted || dayData.pmCompleted) && !(dayData.amCompleted && dayData.pmCompleted)) {
      return (
        <>
          ...
          {dayData.amCompleted && (
            <div className="absolute inset-0 bg-green-200 z-0" style={{ clipPath: 'polygon(0 0, 0% 100%, 100% 0)' }}></div>
          )}
          {dayData.pmCompleted && (
      ...
      <div className="mt-2 flex flex-wrap items-center text-xs text-gray-600">
        ...
        <div className="flex items-center mr-4">
          <div className="w-4 h-4 bg-white border mr-1 relative overflow-hidden">
            <div className="absolute inset-0 bg-green-200" style={{ clipPath: 'polygon(0 0, 0% 100%, 100% 0)' }}></div>
          </div>
